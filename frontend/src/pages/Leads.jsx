...
export default function Leads() {
  ...
  const handleStatusChange = async (leadId, nextStatus) => {
    ...
    setLeads((prev) => prev.map((lead) => (lead._id === leadId ? { ...lead, status: nextStatus } : lead)));
  ...
  const emptyState = useMemo(() => !isLoading && leads.length === 0, [isLoading, leads.length]);
    ...
    <div className="min-h-screen bg-gray-50 p-4 sm:p-6 lg:p-8">
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-gray-600">Manage incoming opportunities and keep your pipeline moving.</p>
        </div>
        <button
          type="button"
      ...
      <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
        {isLoading ? (
          ...
          <div className="flex h-48 flex-col items-center justify-center px-6 text-center">
            ...
            <p className="mt-2 text-sm text-gray-500">Add one to get started!</p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500">Lead Name</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500">Email</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500">Status</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500">Created Date</th>
                ...
                {leads.map((lead) => (
                  <tr key={lead._id || lead.email} className="hover:bg-gray-50">
                    <td className="px-4 py-3 text-sm font-medium text-gray-900">
                      <Link to={`/leads/${lead._id}`} className="text-blue-600 hover:underline">
                        {lead.name}
                      </Link>
                    </td>
                    ...
                    <td className="px-4 py-3 text-sm text-gray-600">{new Date(lead.createdAt).toLocaleDateString()}</td>
                    <td className="px-4 py-3 text-sm">
                      <div className="relative">
                        ...
                        {activeMenuId === lead._id ? (
                          <div className="absolute right-0 z-10 mt-2 w-32 rounded-lg border border-gray-200 bg-white py-1 shadow-lg">
                            <button
                              type="button"
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Add New Lead">
        <AddLeadForm onSuccess={handleAddLead} onClose={() => setIsModalOpen(false)} />
      </Modal>
